...
import { ref } from 'vue';
import { CASTLE_COUNT } from '../constants/poi.js';
...
export const usePoiStore = defineStore('poi', () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      while (attempts < 1000 && !placed) {
        ...
        if (terrain[ty]?.[tx] > -0.05) {
          pois.value.push({ id: i, type: 'castle', position: { x, y }, discovered: false });
          placed = true;
        }
        attempts++;
      }
  ...
  function checkDiscovery(playerPosition) {
    pois.value.forEach(poi => {
      if (!poi.discovered && Math.abs(poi.position.x - playerPosition.x) < 10 && Math.abs(poi.position.y - playerPosition.y) < 10) {
        poi.discovered = true;
        console.log('🏰 Descubierto punto de interés:', poi.type, 'en', poi.position);
      }
    });
  }
